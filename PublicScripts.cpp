...
#define auto_str unique_ptr<char[]>
#else
struct auto_str
{
	char* s_;
...
void cScr_QuickText::DisplayText(void)
{
	...
	if (pPS->Possessed(ObjId(), "Book"))
	{
		...
		auto_str szBookFile (new char[strlen(mpBook) + 10]);
...
long cScr_TrapQVarMis::OnSim(sSimMsg* pSimMsg, cMultiParm& mpReply)
{
	if (pSimMsg->fStarting)
	{
		...
		{
			char* pszName = NULL;
			auto_str pszParam (GetQVarParams(ObjId(), NULL, NULL, &pszName));
			if (pszName)
			{
				int iInit;
				if (pszInit[0] == '$')
...
long cScr_TrapQVarMis::OnSwitch(bool bTurnOn, sScrMsg* pMsg, cMultiParm& mpReply)
{
	...
	int iValue = 0;
	char* pszName = NULL;
	auto_str pszParam (GetQVarParams(ObjId(), &cOperation, &iValue, &pszName));
	if (pszName)
...
#endif
	char* pszQVar = NULL;
	auto_str pszParam (GetQVarParams(ObjId(), NULL, NULL, &pszQVar));
	if (pszQVar)
	{
		SService<IQuestSrv> pQS(g_pScriptManager);
...
long cScr_TrigQVar::OnQuestChange(sQuestMsg* pQuestMsg, cMultiParm& mpReply)
{
	...
	int iArg = 0;
	char* pszQVar = NULL;
	auto_str pszParam (GetQVarParams(ObjId(), &cOp, &iArg, &pszQVar));
	if (pszQVar)
	{
